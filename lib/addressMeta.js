 ...
 *
 * The numeric id is the only stable part, so that is what we key on. The frontend now sends the
 * bare numeric key too (the full gid must never travel in a URL — it carries the token), so a
 * digits-only value is already the key.
 */
const addressKey = (gid) => {
  const s = String(gid || '');
  if (/^\d+$/.test(s)) return s;
  const match = s.match(/MailingAddress\/(\d+)/);
...
function normalise(meta = {}) {
  /*
    THE BLANK IS CAUGHT BEFORE THE COERCION, NOT AFTER.

    `Number(null)` is 0, `Number('')` is 0, and 0 here means GROUND FLOOR -- the cheapest tier, no
    floor multiplier, and priced with full confidence. So a plain `Number(meta.floor)` turned "never
    asked" into a real, billable answer: the frontend deliberately sends `null` for an unanswered
    floor (see the note beside payload.floor in the addresses page) and this threw that away. A
    fifth-floor walk-up was billed as a ground-floor drop and dispatched as one -- Rs 300 against
    Rs 4,800 of actual labour, with nobody phoned because the quote looked settled.

    The old guard was on NaN and range, which correctly stops 0 collapsing to null. This is the same
    mistake in the other direction, and `liftAvailable` below already got it right.

    Only a number or a non-blank numeric string counts as an answer. Typing the check rather than
    testing the coerced result also drops the rest of the family this had: `Number(true)` is 1 (a
    malformed client would have booked floor 1) and `Number([])` is 0, ground floor again.
  */
  const raw = meta.floor;
  const floorGiven = typeof raw === 'number' || (typeof raw === 'string' && raw.trim() !== '');
  const floorRaw = floorGiven ? Number(raw) : NaN;
  return {
